...
' this code here will tell use the bandwidth
oNew = Trim(Format(m_objIpHelper.BytesReceived, "###,###,###,###"))
 


'If oOld > oNew Then Exit Function
tValue = oNew - oOld
Label10.Caption = oNew
Label11.Caption = oOld
Label9.Caption = tValue
Label8.Caption = (tValue / 1000)
' we set the old value to the new value - if we try to call
' the value again by the time we exit this function
' the value of the octets rec may have changed and we will
' not get an accurate bandwidth amount
oOld = oNew
End Function

Private Sub Timer2_Timer()
' the timer is set to update every second value = 1000
bandwidth
End Sub
